...
\end{columns}
\vfill
{\tiny\color{gray} Gumudavelli et al.\ (2007), \textit{J.\ Food Sci.}\ 72:M254--62;\quad Humphrey (1994), \textit{Int J Food Microbiol}\ 21:31--40.}
\end{frame}
...
\end{columns}
\vfill
{\tiny\color{gray} Zwietering et al.\ (1991), \textit{Appl Environ Microbiol}\ 57:1094--101;\quad Baranyi \& Roberts (1994), \textit{Int J Food Microbiol}\ 23:277--94.}
\end{frame}
...
\end{columns}
\vfill
{\tiny\color{gray} Data source: Gumudavelli et al.\ (2007), Fig.\,5c --- sinusoidal temperature profile.}
\end{frame}
...
{\footnotesize\textbf{Fig.\,1.} Forward prediction with initial guesses.\\Black: observed; Blue: predicted; Red: temperature.}
\end{column}
\begin{column}{0.48\textwidth}
\centering
...
{\footnotesize\textbf{Fig.\,2.} Scaled sensitivity coefficients at initial guesses. All 5 parameters show distinct patterns.}
\end{column}
\end{columns}
\vspace{0.2em}
{\small $T_{\min}$, $T_{\max}$ have negligible SSC $\rightarrow$ fixed at literature values.}
\vfill
{\tiny\color{gray} SSC: $\text{SSC}_{ij} = p_j \cdot \partial y_i / \partial p_j$;\quad Beck \& Arnold (1977), Ch.\,5.}
\end{frame}
...
\vfill
{\tiny\color{gray} Estimated via MATLAB \texttt{nlinfit} (Levenberg--Marquardt);\quad Beck \& Arnold (1977).}
\end{frame}
...
{\footnotesize\textbf{Fig.\,3.} OLS fitted curve with 95\% asymptotic confidence band (dark) and prediction band (light). Black dots: observed; Blue: predicted; Red dashed: temperature.}
\vfill
{\tiny\color{gray} CB and PB computed via MATLAB \texttt{nlpredci};\quad Gumudavelli et al.\ (2007).}
\end{frame}
...
{\footnotesize\textbf{Fig.\,6.} SSC recomputed with estimated parameters. Patterns consistent with initial SSC.}
\end{column}
...
\end{columns}
\vfill
{\tiny\color{gray} Beck \& Arnold (1977), p.\,29--30.}
\end{frame}
...
{\footnotesize\textbf{Fig.\,7.} Delta criterion $\det(X^TX)$ vs.\ last measurement time. Increasing: more data helps.}
\end{column}
\begin{column}{0.48\textwidth}
\centering
...
{\footnotesize\textbf{Fig.\,8.} $C_{ii}$ curves (log scale). Decreasing: parameters become more precise.}
\end{column}
\end{columns}
\vspace{0.2em}
{\small Key measurement window: exponential growth phase ($\sim$2--8\,hr).}
\vfill
{\tiny\color{gray} Beck \& Arnold (1977), p.\,419--443.}
\end{frame}
...
{\footnotesize\textbf{Fig.\,9.} Bootstrap CB (green) and PB (pink) from 1000 residual-resampling iterations.}
\end{column}
\end{columns}
\vfill
{\tiny\color{gray} Residual resampling bootstrap;\quad Beck \& Arnold (1977).}
\end{frame}
